...
const aiInsights = [
  {
    type: "prediction",
    message: "TechStart Inc typically pays 3 days late. Consider early reminder.",
    confidence: 89,
  },
  {
    type: "alert",
    message: "Acme Corporation's payment pattern changed — historically pays on time.",
    confidence: 94,
  },
  {
    type: "suggestion",
    message: "Send SMS reminder to StartupXYZ — email open rate is only 12%.",
    confidence: 76,
  },
];
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <aside className="fixed left-0 top-0 h-full w-64 bg-white border-r border-gray-200 z-40 hidden lg:block">
        ...
        <div className="absolute bottom-0 left-0 right-0 p-4">
          <div className="bg-gradient-to-r from-primary-500 to-success-500 rounded-xl p-4 text-white">
      ...
      <main className="lg:ml-64 min-h-screen">
        ...
        <header className="bg-white border-b border-gray-200 px-6 py-4 flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-gray-500">Welcome back! You have 2 overdue invoices.</p>
          </div>
        ...
        <div className="p-6 space-y-6">
            ...
            {[
              ...
              <div key={i} className="bg-white rounded-xl p-5 border border-gray-100 shadow-sm">
                ...
                <div className={`flex items-center gap-1 mt-1 text-sm ${
                  stat.trend === "up" ? "text-success-600" : "text-success-600"
                }`}>
                  {stat.trend === "up" ? (
          ...
          {/* AI Insights */}
          <div className="bg-gradient-to-r from-primary-50 to-success-50 rounded-xl p-5 border border-primary-100">
              ...
              {aiInsights.map((insight, i) => (
                <div key={i} className="flex items-start gap-3 bg-white rounded-lg p-3">
                  <div className={`w-2 h-2 rounded-full mt-2 flex-shrink-0 ${
                    insight.type === "prediction" ? "bg-primary-500" :
                    insight.type === "alert" ? "bg-warning-500" : "bg-success-500"
                  }`} />
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="relative">
                  <select
                    ...
                    className="pl-3 pr-8 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary-500 appearance-none"
                  >
              ...
              <table className="w-full">
                <thead className="bg-gray-50">
                  <tr>
                    ...
                    <th className="px-5 py-3 text-left text-xs font-semibold text-gray-500 uppercase">AI Action</th>
                ...
                <tbody className="divide-y divide-gray-100">
                  {filteredInvoices.map((invoice) => (
                    <tr key={invoice.id} className="hover:bg-gray-50 transition-colors">
                      <td className="px-5 py-4 text-sm font-medium text-gray-900">{invoice.id}</td>
                      <td className="px-5 py-4 text-sm text-gray-700">{invoice.client}</td>
                      ...
                      <td className="px-5 py-4">
                        <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium ${
                          ...
                          <span className={`w-1.5 h-1.5 rounded-full ${statusColors[invoice.status]}`} />
                          {statusLabels[invoice.status]}
                          {invoice.status === "overdue" && (
                            <span className="text-danger-500">({invoice.daysOverdue}d)</span>
                          )}
                        </span>
                      </td>
                      ...
                      <td className="px-5 py-4 text-right">
                        <div className="flex items-center justify-end gap-2">
                          <button className="p-2 text-gray-400 hover:text-primary-600 transition-colors" title="Send reminder now">
                            <Send className="w-4 h-4" />
                          </button>
                          <button className="p-2 text-gray-400 hover:text-primary-600 transition-colors" title="Mark as paid">
          ...
          <div className="grid sm:grid-cols-3 gap-4">
            {[
              { label: "Send Bulk Reminders", desc: "AI-optimized for 5 overdue invoices", icon: Send },
              { label: "Generate Report", desc: "Monthly DSO & payment analysis", icon: TrendingUp },
              { label: "Configure AI", desc: "Adjust tone & reminder schedule", icon: Settings },
            ].map((action, i) => (
              <button
                key={i}
